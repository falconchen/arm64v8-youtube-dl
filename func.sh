...
function yt-dlp-ng() {
  ...
  docker run -i --rm -v "${localDir}:/data" falconchen/arm64v8-yt-dlp \
    ...
    -f '22/(bv*[vcodec^=vp9][height<=1024]+ba[acodec=opus])/bv*+ba/22/137+ba[ext=m4a]/137+ba/302+ba[ext=m4a]/302+ba/bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo[ext=mp4]+(258/256/140)/bestvideo[ext=webm]+(250/249)/bestvideo[ext=webm]+bestaudio/mp4/best' \
    "$@"
}
